...
@api_bp.route('/freight/items/export.xlsx', methods=['GET'])
@login_required
def export_freight_items_xlsx():
    """Export freight items as XLSX (reuses CSV logic then converts to Excel)."""
    if Workbook is None:
        return jsonify({'error': 'openpyxl is not installed on the server'}), 500
    try:
        resp = export_freight_items_csv()
        ...
        return Response(out.read(),
                        ...
                        headers={'Content-Disposition': 'attachment; filename="freight_items.xlsx"'})
    except Exception as exc:
        return jsonify({'error': str(exc)}), 500
